...
if not os.path.exists("screenshot_canvas"):
    os.makedirs("screenshot_canvas")
            ...
            if fingers[1] and fingers[2]:
                ...
                if y1 < 100:
                    if 100 < x1 < 190:
                        ...
                        cv2.imwrite(os.path.join("screenshot", file_name), img)
                        cv2.imwrite(os.path.join("screenshot_canvas", f"{file_name[:-4]}_canvas.png"), imgCanvas)
                        # Creating a composite image of the frame and the canvas
                        screenshot = cv2.add(img, imgCanvas)
                        cv2.imwrite(os.path.join("painter", file_name), screenshot)
